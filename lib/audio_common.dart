...
import 'package:english_sub/utils.dart';
    ...
    return Stack(
      children: [
        ...
        Positioned(
          ...
          bottom: 0.0,
          child: Text(
              RegExp(r'((^0*[1-9]\d*:)?\d{2}:\d{2})\.\d+$').firstMatch("${widget.position}")?.group(1) ??
                  '${widget.position}',
...
class ControllerButtons extends StatefulWidget {
  ControllerButtons({Key? key, required this.audioHandler}) : super(key: key);
...
class _ControllerButtonsState extends State<ControllerButtons> {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        // IconButton(onPressed: () => widget.audioHandler.skipToPrevious(), icon: Icon(Icons.arrow_back)),
        IconButton(onPressed: () => widget.audioHandler.pause(), icon: Icon(Icons.pause)),
        // IconButton(onPressed: () => widget.audioHandler.play(), icon: Icon(Icons.play_arrow)),
        // IconButton(onPressed: () => widget.audioHandler.skipToNext(), icon: Icon(Icons.arrow_forward))
      ],
    );
  }
}
